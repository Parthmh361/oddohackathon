...
export default function Receipts() {
  ...
  const fetchReceipts = async () => {
    try {
      const response = await receiptsAPI.getAll({ status: status || undefined });
        ...
        <div className="p-8">
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Incoming stock from suppliers</p>
            </div>
                  ...
                  {loading ? (
                    ...
                    receipts.map((receipt) => (
                      <tr key={receipt.id} className="border-b hover:bg-gray-50 transition">
                        <td className="py-3 px-4 font-semibold text-gray-900">{receipt.receiptNumber}</td>
                        <td className="py-3 px-4 text-gray-900">{receipt.supplierName}</td>
